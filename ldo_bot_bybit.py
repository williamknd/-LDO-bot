...
def get_signal(streak):
    last = streak[-1]
    if last <= -STREAK_TRIGGER: return 1   # LONG após 5 vermelhas
    if last >= STREAK_TRIGGER:  return -1  # SHORT após 5 verdes
    return 0
...
.badge.open{background:#1a0a2e;color:#a855f7}.badge.buy{background:#003322;color:#00ff88}.badge.sell{background:#330011;color:#ff4466}
...
async function update(){
  try{
    ...
    document.getElementById('subtitle').textContent=(d.testnet?'TESTNET':'MAINNET')+' | LDOUSDT | SL: 1.5% | Hold: 120 velas';
...
# ── LOOP PRINCIPAL ───────────────────────────────────
def run():
    log.info(f"LDO Bot | {SYMBOL} | Testnet: {TESTNET} | Qty: {QTY} | SL: 1.5% | Hold: 120 velas")
    set_leverage()
