"""
LDO 5-Red Candle Reversal Bot - Bybit + Dashboard
...
# ── CONFIG ──────────────────────────────────────────
API_KEY        = os.environ.get("LDO_API_KEY", "")
API_SECRET     = os.environ.get("LDO_API_SECRET", "")
TESTNET        = os.environ.get("TESTNET", "true").lower() == "true"
SYMBOL         = os.environ.get("LDO_SYMBOL", "LDOUSDT")
CATEGORY       = "linear"
TIMEFRAME      = "5"
QTY            = os.environ.get("LDO_QTY", "2")
LEVERAGE       = int(os.environ.get("LEVERAGE", "1"))
STOP_LOSS_PCT  = float(os.environ.get("LDO_STOP_LOSS_PCT", "0.015"))  # 1.5%
...
PORT           = int(os.environ.get("LDO_PORT", "8081"))

logging.basicConfig(level=logging.INFO, format="%(asctime)s | %(levelname)s | %(message)s")
log = logging.getLogger("LDOBot")

if not API_KEY or not API_SECRET:
    log.error("LDO_API_KEY e LDO_API_SECRET nao configurados!")
    exit(1)
...
def open_position(side, price):
    ...
    try:
        ...
        state["trades"].append({"type": side, "entry": price, "sl": sl, "time": datetime.now().strftime("%H:%M:%S"), "result": "ABERTA", "exit": None, "pnl": None})
...
h1{font-size:1.8rem;font-weight:800;letter-spacing:4px;text-transform:uppercase;color:#a855f7;text-shadow:0 0 20px #a855f755;margin-bottom:4px}
...
.card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,#a855f744,transparent)}
.card-label{font-size:.65rem;letter-spacing:2px;text-transform:uppercase;color:#4a7a9b;margin-bottom:8px}
.card-value{font-family:'Share Tech Mono';font-size:1.5rem;font-weight:700;color:#a855f7}
...
.winrate-fill{height:100%;background:linear-gradient(90deg,#a855f7,#00d4ff);border-radius:3px;transition:width .5s}
.section-title{font-size:.7rem;letter-spacing:3px;text-transform:uppercase;color:#4a7a9b;margin:20px 0 10px}
.candles{display:flex;gap:3px;align-items:flex-end;height:80px;background:#0a1628;border-radius:8px;padding:8px;border:1px solid #0d2540}
...
.badge.open{background:#0d2540;color:#a855f7}.badge.buy{background:#003322;color:#00ff88}.badge.sell{background:#330011;color:#ff4466}
...
<div class="grid">
  ...
  <div class="card"><div class="card-label">Preco LDO</div><div id="price" class="card-value">--</div></div>
  <div class="card"><div class="card-label">Streak</div><div id="streak" class="card-value">--</div></div>
...
<div class="section-title">Ultimas 20 Velas (5min)</div>
...
async function update(){
  try{
    ...
    document.getElementById('subtitle').textContent=(d.testnet?'TESTNET':'MAINNET')+' | '+d.symbol+' | SL: 1.5%';
    ...
    const sk=document.getElementById('streak');sk.textContent=(d.streak>0?'+':'')+d.streak;
    sk.className='card-value '+(d.streak<0?'red':d.streak>0?'green':'');
    ...
    document.getElementById('wins').textContent=d.wins;document.getElementById('losses').textContent=d.losses;
    const total=d.wins+d.losses;const wr=total>0?Math.round(d.wins/total*100):0;
    document.getElementById('winrate').textContent=wr+'%';document.getElementById('winrate-bar').style.width=wr+'%';
    ...
    const cc=document.getElementById('candles');cc.innerHTML='';
    if(d.recent_candles&&d.recent_candles.length){
      const highs=d.recent_candles.map(c=>c.h);const lows=d.recent_candles.map(c=>c.l);
      const maxH=Math.max(...highs);const minL=Math.min(...lows);const range=maxH-minL||1;
      d.recent_candles.forEach(c=>{
        const isGreen=c.c>=c.o;const bodyH=Math.max(4,Math.abs(c.c-c.o)/range*60);
        ...
        b.style.height=bodyH+'px';b.style.marginTop='auto';w.appendChild(b);cc.appendChild(w);
      });
    }
...
# ── LOOP PRINCIPAL ───────────────────────────────────
def run():
    log.info(f"LDO Bot | {SYMBOL} | Testnet: {TESTNET} | Qty: {QTY} | SL: 1.5%")
    set_leverage()
    ...
    while True:
        try:
            ...
            log.info(f"Preco: {price:.4f} | Streak: {streak[-1]:+d} | Sinal: {state['signal']}")
